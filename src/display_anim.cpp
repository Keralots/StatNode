...
void drawCompletionAnim(lgfx::LovyanGFX& gfx, int16_t cx, int16_t cy, bool reset) {
  ...
  if (elapsed < 400) {
    ...
    if (prevRing > 0 && prevRing != r) {
      gfx.drawArc(cx, cy, prevRing, prevRing - 3, 0, 360, CLR_BG);
    }
    gfx.drawArc(cx, cy, r, r - 3, 0, 360, CLR_GREEN);
    prevRing = r;
  }
  // Phase 2 (400-600ms): settle to final ring
  else if (elapsed < 600) {
    gfx.drawArc(cx, cy, finalR, finalR - 4, 0, 360, CLR_GREEN);
  }
  // Phase 3 (600ms+): static ring + large checkmark, done
  else {
    gfx.drawArc(cx, cy, finalR, finalR - 4, 0, 360, CLR_GREEN);
    // Clear center for checkmark
    gfx.fillCircle(cx, cy, finalR - 5, CLR_BG);
    // Draw 32x32 checkmark centered
    drawIcon32(gfx, cx - 16, cy - 16, icon_check_32, CLR_GREEN);
    completionDone = true;
  }
}
